...
#Q7
select r.code, t.name, m.day, m.start_time, m.end_time, m.weeks_binary from courses as c, classes as cl, rooms as r, terms as t, meetings as m where c.term_id=t.id and cl.course_id=c.id and cl.id=m.class_id and m.room_id=r.id and r.code ILIKE 'K-%' order by r.code, m.start_time, m.end_time;
select count(DISTINCT code) as counter from rooms where code ILIKE 'K-%';

#Q8
select DISTINCT s.code, c.name, m.day, m.start_time, m.end_time, m.weeks_binary from courses as co, terms as t, classes as cl, meetings as m, subjects as s, classtypes as c where co.term_id=t.id and s.id=co.subject_id and co.id=cl.course_id and c.id=cl.type_id  and m.class_id=cl.id and t.id = 5199 order by m.day, s.code, m.start_time;
